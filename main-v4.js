(() => {
  ...
  ui.btns.restart.addEventListener('click', () => { newGame(); refreshInv(); });
  ...
  window.addEventListener('keydown', e=>{
    ...
    if(e.code==='KeyR') { newGame(); refreshInv(); } // quick restart on R
  });
  ...
  function startSimon(){ const seq=[], colors=['#3b82f6','#16a34a','#f59e0b','#ef4444']; const add=()=>seq.push((Math.random()*4)|0); add(); add(); let idx=0,showing=true,showTick=0; state.minigame={type:'simon',seq,colors,idx,showing,showTick,input:[],update(dt){showTick+=dt;if(showing && showTick>700){this.idx++;showTick=0;if(this.idx>=seq.length){this.showing=false;this.idx=0;}}},click(mx,my){if(this.showing)return;const w=200,h=200,x=(W-w)/2,y=(H-h)/2;const quad=(mx<x+w/2?(my<y+h/2?0:2):(my<y+h/2?1:3));this.input.push(quad);const ok=this.input[this.input.length-1]===seq[this.input.length-1];if(!ok) endMinigame(false,"Simon failed. Try another shrine later."); else if(this.input.length===seq.length) endMinigame(true,"Simon cleared! You gained a sigil.");},draw(){ctx.fillStyle='rgba(0,0,0,.7)';ctx.fillRect(0,0,W,H);const w=200,h=200,x=(W-w)/2,y=(H-h)/2;const rects=[[x,y],[x+w/2,y],[x,y+h/2],[x+w/2,y+h/2]];for(let i=0;i<4;i++){ctx.fillStyle=colors[i];ctx.fillRect(rects[i][0],rects[i][1],w/2,h/2);}if(this.showing){const hi=seq[Math.min(this.idx,seq.length-1)];ctx.fillStyle='rgba(255,255,255,.6)';const rx=rects[hi][0],ry=rects[hi][1];ctx.fillRect(rx,ry,w/2,h/2);}else{drawCenterText("Repeat the sequence by clicking the colored squares.",80);}}}; }
  function startClicker(){ const targets=[],total=5; let captured=0; for(let i=0;i<total;i++){targets.push({x:(60+Math.random()*(W-120))|0,y:(60+Math.random()*(H-120))|0,r:18});} const t0=performance.now(), lim=10000; state.minigame={type:'clicker',update(){if(performance.now()-t0>lim) endMinigame(false,"You ran out of time.");},click(mx,my){for(let i=0;i<targets.length;i++){const t=targets[i];if(Math.hypot(mx-t.x,my-t.y)<=t.r){targets.splice(i,1);captured++;break;}}if(captured>=total) endMinigame(true,"Great aim! You gained a sigil.");},draw(){ctx.fillStyle='rgba(0,0,0,.7)';ctx.fillRect(0,0,W,H);drawCenterText("Click all the circles in 10 seconds!",80);for(const t of targets){ctx.beginPath();ctx.arc(t.x,t.y,t.r,0,Math.PI*2);ctx.fillStyle='#eab308';ctx.fill();}const left=Math.max(0,((lim-(performance.now()-t0))/1000)|0);drawCenterText(`Targets left: ${total-captured}  |  Time: ${left}s`,120);}}; }
  ...
  function gameOver(text){
    ...
    // Drawn on canvas only; restart with R or Restart button
  ...
  function draw(){
    ...
    if (state.won){
      ...
      ctx.fillStyle='#fff7ed'; ctx.font='bold 72px system-ui,Segoe UI,Roboto,Arial';
      const m=ctx.measureText('THE END'); ctx.fillText('THE END', (W-m.width)/2, H/2);
    }
    if (state.lost){
      ctx.fillStyle='rgba(0,0,0,.75)'; ctx.fillRect(0,0,W,H);
      ctx.fillStyle='#ef4444'; ctx.font='bold 64px system-ui,Segoe UI,Roboto,Arial';
      const over='GAME OVER'; let m=ctx.measureText(over); ctx.fillText(over,(W-m.width)/2, H/2 - 20);
      ctx.fillStyle='#e5e7eb'; ctx.font='20px system-ui,Segoe UI,Roboto,Arial';
      const msg=state.gameOverText || ''; m=ctx.measureText(msg); ctx.fillText(msg,(W-m.width)/2, H/2 + 12);
      const hint='Press R to Restart'; m=ctx.measureText(hint); ctx.fillText(hint, (W-m.width)/2, H/2 + 40);
    }
